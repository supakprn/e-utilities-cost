e-utilities-cost
![CI](https://github.com/supakprn/e-utilities-cost/actions/workflows/ci.yml/badge.svg)
...
โปรเจกต์นี้ใช้ GitHub Actions (ไฟล์ `.github/workflows/ci.yml`) รันอัตโนมัติทุกครั้งที่ push หรือเปิด Pull Request เข้า branch `main` ประกอบด้วย 3 job:
Job	ทำอะไร
`backend`	ติดตั้ง dependencies ของ backend และเช็คว่าโค้ดโหลดได้โดยไม่ error
`frontend`	ติดตั้ง dependencies ของ frontend และรัน `npm run build` ให้แน่ใจว่า build ผ่าน
`docker-build`	build Docker image ของทั้ง backend และ frontend เพื่อยืนยันว่า Dockerfile ใช้งานได้ (ไม่ push ขึ้น Docker Hub อัตโนมัติ — การ push ทำแบบ manual ตามขั้นตอนด้านบน)
ดูสถานะการรันล่าสุดได้ที่แท็บ Actions ของ repo
API หลัก
...
Changelog
2026-08-10
เพิ่ม GitHub Actions CI pipeline (`.github/workflows/ci.yml`) — build/test อัตโนมัติทุกครั้งที่ push หรือเปิด PR เข้า `main`
